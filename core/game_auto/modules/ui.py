...
class UiSetup:
    ...
    def setup_ui(main_win):
        ...
        # 5. 회수 시작좌표 (S)
        lbl_s_pt = QLabel("회수 시작좌표:")
        lbl_s_pt.setStyleSheet(lbl_style)
        
        s_pt_box = QHBoxLayout()
        s_pt_box.setSpacing(5)
        s_pt_box.setContentsMargins(0, 0, 0, 0)
        
        lbl_sx = QLabel("X:")
        lbl_sx.setStyleSheet("color: #8a99af; font-size: 11px;")
        main_win.txt_pingpong_recovery_start_x = QLineEdit("-1")
        main_win.txt_pingpong_recovery_start_x.setStyleSheet(input_style)
        main_win.txt_pingpong_recovery_start_x.setFixedWidth(40)
        main_win.txt_pingpong_recovery_start_x.setValidator(QIntValidator(-1, 9999))
        main_win.txt_pingpong_recovery_start_x.setObjectName("recovery_start_x")
        main_win.txt_pingpong_recovery_start_x.editingFinished.connect(main_win.settings_manager.save_settings_silently)
        
        lbl_sy = QLabel("Y:")
        lbl_sy.setStyleSheet("color: #8a99af; font-size: 11px;")
        main_win.txt_pingpong_recovery_start_y = QLineEdit("-1")
        main_win.txt_pingpong_recovery_start_y.setStyleSheet(input_style)
        main_win.txt_pingpong_recovery_start_y.setFixedWidth(40)
        main_win.txt_pingpong_recovery_start_y.setValidator(QIntValidator(-1, 9999))
        main_win.txt_pingpong_recovery_start_y.setObjectName("recovery_start_y")
        main_win.txt_pingpong_recovery_start_y.editingFinished.connect(main_win.settings_manager.save_settings_silently)
        
        s_pt_box.addWidget(lbl_sx)
        s_pt_box.addWidget(main_win.txt_pingpong_recovery_start_x)
        s_pt_box.addWidget(lbl_sy)
        s_pt_box.addWidget(main_win.txt_pingpong_recovery_start_y)
        s_pt_box.addStretch()
        
        s_pt_widget = QWidget()
        s_pt_widget.setLayout(s_pt_box)
        
        recovery_grid.addWidget(lbl_s_pt, 5, 0)
        recovery_grid.addWidget(s_pt_widget, 5, 1, 1, 3)
        ...
        recovery_grid.addWidget(lbl_t_pt, 6, 0)
        recovery_grid.addWidget(t_pt_widget, 6, 1, 1, 3)
        ...
        recovery_grid.addWidget(lbl_rec_seq, 7, 0)
        recovery_grid.addWidget(main_win.txt_pingpong_recovery_seq, 7, 1, 1, 3)
        
        # 8. 복귀 간격 & 회수 간격
        lbl_ret_delay = QLabel("복귀 간격(초):")
        lbl_ret_delay.setStyleSheet(lbl_style)
        main_win.txt_pingpong_return_delay = QLineEdit("1.0")
        main_win.txt_pingpong_return_delay.setStyleSheet(input_style)
        main_win.txt_pingpong_return_delay.setFixedWidth(50)
        main_win.txt_pingpong_return_delay.setValidator(QDoubleValidator(0.01, 10.0, 2))
        main_win.txt_pingpong_return_delay.setObjectName("return_delay")
        main_win.txt_pingpong_return_delay.editingFinished.connect(main_win.settings_manager.save_settings_silently)
        
        lbl_rec_delay = QLabel("회수 간격(초):")
        lbl_rec_delay.setStyleSheet(lbl_style)
        main_win.txt_pingpong_recovery_delay = QLineEdit("1.0")
        main_win.txt_pingpong_recovery_delay.setStyleSheet(input_style)
        main_win.txt_pingpong_recovery_delay.setFixedWidth(50)
        main_win.txt_pingpong_recovery_delay.setValidator(QDoubleValidator(0.01, 10.0, 2))
        main_win.txt_pingpong_recovery_delay.setObjectName("recovery_delay")
        main_win.txt_pingpong_recovery_delay.editingFinished.connect(main_win.settings_manager.save_settings_silently)
        
        recovery_grid.addWidget(lbl_ret_delay, 8, 0)
        recovery_grid.addWidget(main_win.txt_pingpong_return_delay, 8, 1)
        recovery_grid.addWidget(lbl_rec_delay, 8, 2)
        recovery_grid.addWidget(main_win.txt_pingpong_recovery_delay, 8, 3)
        
        # 9. 회수 시 텔레포트 사용 체크박스
        main_win.chk_pingpong_recovery_teleport = QCheckBox("회수 시 텔레포트 사용")
        main_win.chk_pingpong_recovery_teleport.setStyleSheet("QCheckBox { color: #c9d1d9; font-weight: bold; font-size: 12px; }")
        main_win.chk_pingpong_recovery_teleport.setChecked(False)
        main_win.chk_pingpong_recovery_teleport.toggled.connect(main_win.settings_manager.save_settings_silently)
        
        recovery_grid.addWidget(main_win.chk_pingpong_recovery_teleport, 9, 0, 1, 4)
        ...
        lbl_build_act = QLabel("동작:")
        lbl_build_act.setStyleSheet(lbl_style)
        main_win.cb_recovery_action_key = QComboBox()
        main_win.cb_recovery_action_key.addItems([
            "TELE_LEFT", "TELE_RIGHT", "TELE_UP", "DROP", 
            "JUMP_LEFT", "JUMP_RIGHT", "JUMP_UP", "WALK_LEFT", "WALK_RIGHT"
        ])
        main_win.cb_recovery_action_key.setStyleSheet("QComboBox { background-color: #0d1117; color: #00d2ff; border: 1px solid #30363d; border-radius: 5px; font-weight: bold; font-size: 11px; padding: 1px 3px; } QComboBox::drop-down { border: none; }")
        main_win.cb_recovery_action_key.setFixedWidth(105)
        
        lbl_build_hold = QLabel("지속(초):")
        lbl_build_hold.setStyleSheet(lbl_style)
        main_win.txt_recovery_action_hold = QLineEdit("0.2")
        main_win.txt_recovery_action_hold.setStyleSheet(input_style)
        main_win.txt_recovery_action_hold.setFixedWidth(40)
        main_win.txt_recovery_action_hold.setValidator(QDoubleValidator(0.01, 5.0, 2))
        
        main_win.btn_add_recovery_action = QPushButton("동작 추가")
        main_win.btn_add_recovery_action.setStyleSheet("font-size: 11px; padding: 2px; border-radius: 5px; background-color: #0288d1; color: white; font-weight: bold;")
        main_win.btn_add_recovery_action.setFixedSize(70, 26)
        
        def add_recovery_action_to_seq():
            act = main_win.cb_recovery_action_key.currentText()
            hold = main_win.txt_recovery_action_hold.text().strip() or "0.2"
            curr = main_win.txt_pingpong_recovery_seq.text().strip()
            new_act = f"{act}:{hold}"
            if curr:
                main_win.txt_pingpong_recovery_seq.setText(f"{curr}, {new_act}")
            else:
                main_win.txt_pingpong_recovery_seq.setText(new_act)
            main_win.settings_manager.save_settings_silently()
            
        main_win.btn_add_recovery_action.clicked.connect(add_recovery_action_to_seq)
        
        builder_layout.addWidget(lbl_build_act)
        builder_layout.addWidget(main_win.cb_recovery_action_key)
        builder_layout.addWidget(lbl_build_hold)
        builder_layout.addWidget(main_win.txt_recovery_action_hold)
        builder_layout.addWidget(main_win.btn_add_recovery_action)
        builder_layout.addStretch()
        
        recovery_grid.addWidget(builder_widget, 10, 0, 1, 4)
        ...
        recovery_grid.addWidget(lbl_hint, 11, 0, 1, 4)
        ...
        def toggle_recovery_inputs(checked):
            ...
            main_win.txt_pingpong_recovery_start_x.setEnabled(checked)
            main_win.txt_pingpong_recovery_start_y.setEnabled(checked)
            main_win.txt_pingpong_teleport_x.setEnabled(checked)
            main_win.txt_pingpong_teleport_y.setEnabled(checked)
            main_win.txt_pingpong_return_delay.setEnabled(checked)
            main_win.txt_pingpong_recovery_delay.setEnabled(checked)
            main_win.chk_pingpong_recovery_teleport.setEnabled(checked)
            main_win.cb_recovery_action_key.setEnabled(checked)
            main_win.txt_recovery_action_hold.setEnabled(checked)
            main_win.btn_add_recovery_action.setEnabled(checked)
