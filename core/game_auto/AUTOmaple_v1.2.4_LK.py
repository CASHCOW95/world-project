...
class AUTOmapleV9_7(QMainWindow):
            ...
            while not self.stop_threads:
                if self.reg_w > 5:
                    ...
                    try:
                        ...
                        cv2.line(p_img, (self.x_min, 0), (self.x_min, self.reg_h), (59, 130, 246), 2); cv2.line(p_img, (self.x_max, 0), (self.x_max, self.reg_h), (239, 68, 68), 2)
                        if np.any(mask):
                            M = cv2.moments(mask)
    ...
    def status_update_loop(self):
        while not self.stop_threads:
            try:
                ...
                self.signals.status_signal.emit({"uptime": f"{up//3600:02d}:{(up%3600)//60:02d}:{up%60:02d}", "total_uptime": f"{total//3600:02d}:{(total%3600)//60:02d}:{total%60:02d}", "cpu": int(psutil.cpu_percent()), "ram": int(psutil.virtual_memory().percent), "actions": self.actions_cnt, "err_cnt": self.err_cnt})
            except: pass
            time.sleep(1.0)
    ...
    def stop_hunting(self): self.is_running = False; self.start_btn.setEnabled(True); self.stop_btn.setEnabled(False); self.total_hunting_time += int(time.time()-self.current_hunt_start); self.set_key_state('left', False); self.set_key_state('right', False)
