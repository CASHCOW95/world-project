...
# 설정 파일 경로
CONFIG_FILE = "hunter_premium_v4_6_config.json"
...
class AutoHunterV4_6:
    def __init__(self, root):
        self.root = root
        self.root.title("오토헌터 프리미엄 v4.6 (Scroll + Anti-Macro)")
        ...
        # 거탐(Anti-Macro) 설정 - 기본값 (초록색 계열 예시)
        self.anti_lower = np.array([40, 100, 100])
        self.anti_upper = np.array([80, 255, 255])
    ...
    def setup_scrollable_ui(self):
        ...
        # 1. 프리뷰 및 영역지정
        group1 = ttk.LabelFrame(container, text=" [ 1. 영역 지정 (F1/F2 or Selector) ] ", padding="10")
        ...
        ttk.Button(btn_f, text="드래그 선택 (Beta)", command=self.open_selector).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        ...
        # 2. 감도 및 기능 ON/OFF
        group2 = ttk.LabelFrame(container, text=" [ 2. 기능 제어 및 감도 ] ", padding="10")
        group2.pack(fill=tk.X, padx=10, pady=5)
        
        check_f = ttk.Frame(group2); check_f.pack(fill=tk.X, pady=5)
        ttk.Checkbutton(check_f, text="이동 스킬(Dash) 사용", variable=self.use_dash).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(check_f, text="거짓말탐지기 대응", variable=self.use_anti_macro).pack(side=tk.LEFT, padx=20)

        ttk.Label(group2, text="노란색 허용 마진:").pack(anchor=tk.W)
        ...
        # 4. 범위 및 정보
        group4 = ttk.LabelFrame(container, text=" [ 4. 활동 범위 및 단축키 ] ", padding="10")
        ...
        ttk.Label(group4, text=f"공격:{self.KEY_ATTACK.upper()} | 이동:{self.KEY_DASH.upper()} | 점프:{self.KEY_JUMP.upper()}", foreground="#00adb5", font=("Malgun Gothic", 9, "bold")).pack(pady=(10, 0))
    ...
    def open_selector(self):
        ...
        self.sel_canvas = tk.Canvas(self.selector, cursor="cross", bg="grey")
        ...
        self.log("마우스로 영역을 드래그하세요.")

    def on_sel_press(self, e):
        self.start_x, self.start_y = e.x, e.y
        self.rect = self.sel_canvas.create_rectangle(self.start_x, self.start_y, e.x, e.y, outline="cyan", width=3)

    def on_sel_drag(self, e):
        self.sel_canvas.coords(self.rect, self.start_x, self.start_y, e.x, e.y)

    def on_sel_release(self, e):
        w, h = abs(e.x - self.start_x), abs(e.y - self.start_y)
        if w > 5 and h > 5:
            self.reg_l.set(min(self.start_x, e.x))
            self.reg_t.set(min(self.start_y, e.y))
            self.reg_w.set(w); self.reg_h.set(h)
            self.x_max.set(w - 15)
            self.log(f"영역 설정 완료: {w}x{h}")
        self.selector.destroy()

    def start_coord_detection(self):
        self.log("영역 지정 시작 (F1, F2)")
    ...
    def _detection_worker(self):
        ...
        while True:
            if keyboard.is_pressed('f1'):
                ...
                self.reg_l.set(p.x); self.reg_t.set(p.y)
                self.log(f"좌상단: ({p.x}, {p.y})")
                    ...
                    try:
                        ...
                        if np.any(mask):
                            ...
                            if M["m00"] > 0:
                                ...
                                cv2.circle(p_img, (cx, cy), 8, (0, 255, 0), -1)
    ...
    def anti_macro_loop(self):
        """전체 화면에서 특정 도형(거탐)을 감지하고 마우스를 추적하는 루프"""
        with mss.mss() as sct:
            while True:
                if self.use_anti_macro.get():
                    # 거탐은 보통 화면 중앙 근처에 뜨므로 전체 화면을 다 보기보다는 중앙 70% 정도만 스캔
                    ...
                    try:
                        ...
                        # 거탐용 특정 색상 범위 (여기서는 초록색 예시, 필요시 조정)
                        mask = cv2.inRange(hsv, self.anti_lower, self.anti_upper)
                        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                        
                        found = False
                        for cnt in contours:
                            if cv2.contourArea(cnt) > 200: # 너무 작은 노이즈 무시
                                M = cv2.moments(cnt)
                                if M["m00"] > 0:
                                    cx = int(M["m10"] / M["m00"])
                                    cy = int(M["m01"] / M["m00"])
                                    
                                    if not self.is_anti_macro_working:
                                        self.log("[위험] 거탐 감지! 추적 시작")
                                        self.is_anti_macro_working = True
                                    
                                    # 마우스 추적 (약간의 랜덤 오프셋으로 인간미 부여)
                                    pyautogui.moveTo(cx + random.randint(-2, 2), cy + random.randint(-2, 2), duration=0.1)
                                    found = True
                                    break
                        ...
                        pass
                time.sleep(0.05) # 추적 정밀도를 위해 빠르게 반복
    ...
    def toggle_running(self):
        if self.is_running:
            self.is_running = False
            self.start_btn.config(text="사냥 시작 (F12)", bg="#00adb5")
            self.log("사냥 중단")
            pyautogui.keyUp('left'); pyautogui.keyUp('right')
        else:
            self.is_running = True
            self.start_btn.config(text="사냥 중지 (F12)", bg="#ff4b2b")
            self.log("사냥 시작!")
                    ...
                    if np.any(mask):
                        ...
                        if M["m00"] > 0:
                            ...
                            if cx >= self.x_max.get() and cur_dir == 'right':
                                pyautogui.keyUp('right'); cur_dir = 'left'; self.log("우측 전환")
                            elif cx <= self.x_min.get() and cur_dir == 'left':
                                pyautogui.keyUp('left'); cur_dir = 'right'; self.log("좌측 전환")
                            pyautogui.keyDown(cur_dir)
                            
                            now = time.time() * 1000
                            if now - l_att >= self.attack_delay_ms.get():
                                self.press_key(self.KEY_ATTACK); l_att = now
                            
                            # 1. 이동 스킬 사용 체크박스 반영
                            if self.use_dash.get():
                                if now - l_dash >= self.dash_delay_ms.get():
                                    self.press_key(self.KEY_DASH); l_dash = now
                            
                            if (time.time() - l_del_time) >= 300:
                                self.log("[주기] DEL 키 2연타")
                                self.press_key(self.KEY_PERIODIC); time.sleep(0.2); self.press_key(self.KEY_PERIODIC)
                                l_del_time = time.time()
    ...
    def save_current_profile(self):
        ...
        self.profiles_data[n] = {
            ...
            "params": {
                "margin": self.color_margin.get(), 
                "ad": self.attack_delay_ms.get(), 
                "dd": self.dash_delay_ms.get(),
                "use_dash": self.use_dash.get(),
                "use_anti": self.use_anti_macro.get()
            }
        }
    ...
    def apply_profile_data(self, n):
        ...
        self.reg_t.set(d["reg"]["t"]); self.reg_l.set(d["reg"]["l"])
        self.reg_w.set(d["reg"]["w"]); self.reg_h.set(d["reg"]["h"])
        ...
        self.use_anti_macro.set(p.get("use_anti", False))
