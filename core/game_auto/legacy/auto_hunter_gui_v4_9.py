...
# 설정 파일 경로
CONFIG_FILE = "hunter_premium_v4_9_config.json"
...
class AutoHunterV4_9:
    def __init__(self, root):
        self.root = root
        self.root.title("오토헌터 프리미엄 v4.9 (비활성화 모드 Beta)")
        ...
        # 가상 키맵 (비활성화 모드용)
        self.vk_map = {
            "end": VK_END, "space": VK_SPACE, "alt": VK_MENU, 
            "del": VK_DELETE, "left": VK_LEFT, "right": VK_RIGHT
        }
        
        self.base_lower = np.array([245, 230, 0]); self.base_upper = np.array([254, 255, 129])
        self.anti_lower = np.array([0, 0, 200]); self.anti_upper = np.array([180, 40, 255])
        ...
        # 신규 변수
        self.use_dash = tk.BooleanVar(value=True); self.use_anti_macro = tk.BooleanVar(value=False)
        self.hunt_mode = tk.IntVar(value=0) # 0:이동, 1:제자리
        self.stationary_range = tk.IntVar(value=5) # 제자리 범위 (1~10)
        self.use_periodic = tk.BooleanVar(value=True); self.periodic_interval_min = tk.IntVar(value=5)
        self.use_background_mode = tk.BooleanVar(value=False) # 비활성화 모드
        self.window_title_keyword = tk.StringVar(value="") # 찾을 창 제목 키워드
    ...
    def setup_scrollable_ui(self):
        ...
        ttk.Button(btn_f, text="클래식 (F1,F2)", command=self.start_coord_detection).pack(side=tk.LEFT, expand=True, fill=tk.X, padx=2)
        ...
        # 2. 사냥 모드 및 비활성화 설정
        group2 = ttk.LabelFrame(container, text=" [ 2. 사냥 모드 및 비활성화 설정 ] ", padding="10")
        ...
        ttk.Radiobutton(mode_f, text="제자리 사냥", variable=self.hunt_mode, value=1).pack(side=tk.LEFT, padx=10)

        bg_f = ttk.Frame(group2); bg_f.pack(fill=tk.X, pady=5)
        ttk.Checkbutton(bg_f, text="비활성화 모드 (Beta)", variable=self.use_background_mode).pack(side=tk.LEFT, padx=10)
        ttk.Label(bg_f, text="창 제목:").pack(side=tk.LEFT)
        ttk.Entry(bg_f, textvariable=self.window_title_keyword, width=15).pack(side=tk.LEFT, padx=5)

        check_f = ttk.Frame(group2); check_f.pack(fill=tk.X, pady=5)
        ttk.Checkbutton(check_f, text="이동 스킬", variable=self.use_dash).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(check_f, text="거탐 대응", variable=self.use_anti_macro).pack(side=tk.LEFT, padx=15)
        ttk.Checkbutton(check_f, text="주기적 DEL", variable=self.use_periodic).pack(side=tk.LEFT, padx=5)

        # 3. 상세 수치 설정
        group3 = ttk.LabelFrame(container, text=" [ 3. 상세 수치 설정 ] ", padding="10")
        ...
        for label, var, min_v, max_v in [
            ...
            ("제자리 범위 (px):", self.stationary_range, 1, 10),
            ("DEL 입력 주기 (분):", self.periodic_interval_min, 1, 60)
        ]:
            ttk.Label(group3, text=label).pack(anchor=tk.W)
            f = ttk.Frame(group3); f.pack(fill=tk.X, pady=(0, 5))
            ttk.Scale(f, from_=min_v, to=max_v, variable=var, orient=tk.HORIZONTAL).pack(side=tk.LEFT, expand=True, fill=tk.X)
            ttk.Entry(f, textvariable=var, width=6, font=("Malgun Gothic", 10, "bold")).pack(side=tk.LEFT, padx=5)

        # 4. 정보
        group4 = ttk.LabelFrame(container, text=" [ 4. 기타 정보 ] ", padding="10")
        group4.pack(fill=tk.X, padx=10, pady=5)
        ttk.Label(group4, text="※ 비활성화 모드 사용 시 창을 가리지 마세요(최소화 금지).", foreground="#ff4b2b", font=("Malgun Gothic", 8)).pack(anchor=tk.W)
    ...
    # --- Windows API Input Functions ---
    def send_key_bg(self, key_name, duration=0.1):
        if not self.target_hwnd: return
        vk = self.vk_map.get(key_name.lower())
        if not vk: return
        ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYDOWN, vk, 0)
        time.sleep(duration)
        ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYUP, vk, 0)

    def find_window(self):
        keyword = self.window_title_keyword.get()
        if not keyword: return None
        hwnd = ctypes.windll.user32.FindWindowA(None, keyword.encode('euc-kr')) # 창 제목으로 찾기
        if not hwnd:
            # 부분 일치 검색
            def enum_cb(h, l):
                buf = ctypes.create_string_buffer(255)
                ctypes.windll.user32.GetWindowTextA(h, buf, 255)
                if keyword in buf.value.decode('euc-kr', 'ignore'):
                    l.append(h)
                return True
            hwnds = []
            ctypes.windll.user32.EnumWindows(ctypes.WNDENUMPROC(enum_cb), ctypes.py_object(hwnds))
        ...
        with mss.mss() as sct:
            while True:
                if self.use_anti_macro.get():
                    ...
                    try:
                        shot = np.array(sct.grab(region)); img = cv2.cvtColor(shot, cv2.COLOR_BGRA2BGR)
                        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV); mask = cv2.inRange(hsv, self.anti_lower, self.anti_upper)
                        ...
                        for cnt in contours:
                            if cv2.contourArea(cnt) > 150:
                                ...
                                if M["m00"] > 0:
                                    ...
                                    if not self.is_anti_macro_working: self.log("[위험] 거탐 감지!"); self.is_anti_macro_working = True
                                    if self.last_anti_pos: self.last_anti_vel = (cx - self.last_anti_pos[0], cy - self.last_anti_pos[1])
                        ...
                        if not found and self.is_anti_macro_working:
                            if self.anti_lost_time < 20:
                                self.anti_lost_time += 1
                                pred_x = self.last_anti_pos[0] + self.last_anti_vel[0]; pred_y = self.last_anti_pos[1] + self.last_anti_vel[1]
                                self.last_anti_pos = (pred_x, pred_y); pyautogui.moveTo(pred_x, pred_y, duration=0.05)
                            else: self.is_anti_macro_working = False; self.log("[정보] 거탐 종료")
                    except: pass
                time.sleep(0.05)
    ...
    def press_key(self, key):
        if self.use_background_mode.get() and self.target_hwnd:
            self.send_key_bg(key)
        else:
            pyautogui.keyDown(key); time.sleep(random.uniform(0.08, 0.12)); pyautogui.keyUp(key)
    ...
    def toggle_running(self):
        if self.is_running:
            ...
            if not self.use_background_mode.get(): pyautogui.keyUp('left'); pyautogui.keyUp('right')
            else: 
                ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYUP, VK_LEFT, 0)
                ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYUP, VK_RIGHT, 0)
        else:
            if self.use_background_mode.get():
                self.target_hwnd = self.find_window()
                if not self.target_hwnd: self.log("[오류] 창을 찾을 수 없습니다!"); return
                self.log(f"[연결] HWND: {self.target_hwnd}")
    ...
    def hunter_core(self):
        ...
        with mss.mss() as sct:
            while self.is_running:
                if self.is_anti_macro_working: time.sleep(0.5); continue
                region = {"top": self.reg_t.get(), "left": self.reg_l.get(), "width": self.reg_w.get(), "height": self.reg_h.get()}
                try:
                    shot = np.array(sct.grab(region)); img = cv2.cvtColor(shot, cv2.COLOR_BGRA2BGR)
                    m = self.color_margin.get()
                    ...
                    if np.any(mask):
                        ...
                        if M["m00"] > 0:
                            cx = int(M["m10"] / M["m00"])
                            if self.hunt_mode.get() == 1: # 제자리 사냥
                                if start_cx is None: start_cx = cx
                                r = self.stationary_range.get()
                                if cx >= start_cx + r and cur_dir == 'right': 
                                    if self.use_background_mode.get(): ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYUP, VK_RIGHT, 0)
                                    else: pyautogui.keyUp('right')
                                    cur_dir = 'left'
                                elif cx <= start_cx - r and cur_dir == 'left':
                                    if self.use_background_mode.get(): ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYUP, VK_LEFT, 0)
                                    else: pyautogui.keyUp('left')
                                    cur_dir = 'right'
                                if self.use_background_mode.get(): ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYDOWN, self.vk_map[cur_dir], 0)
                                else: pyautogui.keyDown(cur_dir)
                            else: # 이동 사냥
                                start_cx = None
                                if cx >= self.x_max.get() and cur_dir == 'right': 
                                    if self.use_background_mode.get(): ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYUP, VK_RIGHT, 0)
                                    else: pyautogui.keyUp('right')
                                    cur_dir = 'left'
                                elif cx <= self.x_min.get() and cur_dir == 'left':
                                    if self.use_background_mode.get(): ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYUP, VK_LEFT, 0)
                                    else: pyautogui.keyUp('left')
                                    cur_dir = 'right'
                                if self.use_background_mode.get(): ctypes.windll.user32.PostMessageA(self.target_hwnd, WM_KEYDOWN, self.vk_map[cur_dir], 0)
                            ...
                            if self.use_periodic.get() and (time.time() - l_del_time) >= (self.periodic_interval_min.get() * 60):
                                self.press_key(self.KEY_PERIODIC); time.sleep(0.2); self.press_key(self.KEY_PERIODIC); l_del_time = time.time()
                        if abs(cx - last_x) < 2: stuck_cnt += 1
                        else: stuck_cnt = 0; last_x = cx
                        if stuck_cnt > 35: self.press_key("alt"); stuck_cnt = 0 # 점프
                except: break
                time.sleep(0.04)
    ...
    def save_current_profile(self):
        ...
        self.profiles_data[n] = {
            ...
            "params": {
                ...
                "use_per": self.use_periodic.get(), "per_int": self.periodic_interval_min.get(),
                "st_range": self.stationary_range.get(), "bg_mode": self.use_background_mode.get(), "win_title": self.window_title_keyword.get()
            }
        }
    ...
    def apply_profile_data(self, n):
        d = self.profiles_data[n]; p = d["params"]
        ...
        self.use_dash.set(p.get("use_dash", True)); self.use_anti_macro.set(p.get("use_anti", False)); self.hunt_mode.set(p.get("mode", 0))
        self.use_periodic.set(p.get("use_per", True)); self.periodic_interval_min.set(p.get("per_int", 5))
        self.stationary_range.set(p.get("st_range", 5)); self.use_background_mode.set(p.get("bg_mode", False)); self.window_title_keyword.set(p.get("win_title", ""))
...
if __name__ == "__main__":
    root = tk.Tk(); app = AutoHunterV4_9(root); root.mainloop()
