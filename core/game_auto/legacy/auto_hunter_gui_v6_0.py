...
# 설정 파일 경로 (v6.0 전용)
CONFIG_FILE = "hunter_premium_v6_0_config.json"
...
class AutoHunterV6_0:
    def __init__(self, root):
        self.root = root
        self.root.title("오토헌터 프리미엄 v6.0 (최종 안정판)")
        ...
        # 고정 키
        self.KEY_ATTACK = "end"; self.KEY_DASH = "space"; self.KEY_JUMP = "alt"; self.KEY_PERIODIC = "del"
        ...
        self.stationary_range = tk.IntVar(value=5) # 1~10px
    ...
    def setup_scrollable_ui(self):
        ...
        # 1. 미니맵
        group1 = ttk.LabelFrame(container, text=" [ 1. 미니맵 영역 지정 ] ", padding="10")
        ...
        # 2. 기능
        group2 = ttk.LabelFrame(container, text=" [ 2. 기능 제어 ] ", padding="10")
        ...
        for label, var, min_v, max_v in [
            ...
            ("노란색 허용 마진:", self.color_margin, 0, 150)
        ]:
        ...
        # 4. 범위
        group4 = ttk.LabelFrame(container, text=" [ 4. 이동 사냥 범위 ] ", padding="10")
        ...
        self.log_text = tk.Text(container, height=12, bg="#121212", fg="#eeeeee", borderwidth=0, font=("Consolas", 9), padx=10, pady=10)
                        ...
                        for cnt in contours:
                            if cv2.contourArea(cnt) > 150:
                                ...
                                if M["m00"] > 0:
                                    ...
                                    if not self.is_anti_macro_working: self.log("[위험] 거탐 감지! 추적 중"); self.is_anti_macro_working = True
                    ...
                    if np.any(mask):
                        ...
                        if M["m00"] > 0:
                            ...
                            if self.hunt_mode.get() == 1: # 제자리
                                if start_cx is None: start_cx = cx; self.log(f"제자리 시작: {cx}")
                            ...
                            if self.use_periodic.get() and (time.time() - l_del_time) >= (self.periodic_interval_min.get() * 60):
                                self.log(f"[주기] DEL ({self.periodic_interval_min.get()}분)"); self.press_key(self.KEY_PERIODIC); time.sleep(0.2); self.press_key(self.KEY_PERIODIC); l_del_time = time.time()
...
if __name__ == "__main__":
    root = tk.Tk(); app = AutoHunterV6_0(root); root.mainloop()
