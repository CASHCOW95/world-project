...
# 설정 파일 경로
CONFIG_FILE = "hunter_master_config.json"
TARGET_TITLE_PART = "MapleStory Worlds-메이"

class AutoHunterV5_3:
    def __init__(self, root):
        self.root = root
        self.root.title("오토헌터 비활성 v5.3 (MASTERPIECE)")
        ...
        # 설정 변수 (순수 클라이언트 영역 기준)
        self.profiles_data = {}
        self.current_profile_name = tk.StringVar(value="신규 사냥터")
        self.rel_t = tk.IntVar(value=0); self.rel_l = tk.IntVar(value=0)
    ...
    def setup_ui(self):
        ...
        self.profile_combo = ttk.Combobox(row1, state="readonly", width=18)
        ...
        ttk.Button(row2, text="프로필 저장", command=self.save_current_profile).pack(side=tk.RIGHT)

        # 1. 프리뷰
        group1 = ttk.LabelFrame(container, text=" [ 1. 실시간 미니맵 상황 ] ", padding="10")
        group1.pack(fill=tk.X, pady=5)
        ttk.Button(group1, text="미니맵 영역 재설정 (F1, F2)", command=self.start_coord_detection).pack(fill=tk.X, pady=(0, 5))
        self.preview_label = tk.Label(group1, text="미니맵을 찾고 있습니다...", bg="#121212", fg="#00adb5", font=("Malgun Gothic", 8), height=10)
        self.preview_label.pack(fill=tk.X)

        # 2. 정밀도
        group2 = ttk.LabelFrame(container, text=" [ 2. 인식 감도 및 마진 ] ", padding="10")
        ...
        # 3. 키/주기
        group3 = ttk.LabelFrame(container, text=" [ 3. 비활성 스킬 주기 (ms) ] ", padding="10")
        ...
        # 제어
        self.start_btn = tk.Button(container, text="마스터 피스 시작 (F12)", bg="#00adb5", fg="white", font=("Malgun Gothic", 12, "bold"), borderwidth=0, command=self.toggle_running)
                ...
                if img is not None:
                    ...
                    if np.any(mask):
                        ...
                        if M["m00"] > 0:
                            ...
                            cv2.circle(p_img, (cx, cy), 6, (0, 255, 0), -1)
                    
                    # 가이드라인
                    cv2.line(p_img, (self.x_min.get(), 0), (self.x_min.get(), reg["h"]), (0, 173, 181), 1)
                    cv2.line(p_img, (self.x_max.get(), 0), (self.x_max.get(), reg["h"]), (0, 173, 181), 1)
                    self.update_preview(p_img)
                else:
                    self.find_game_window() # 캡처 실패 시 핸들 다시 찾기 시도
            time.sleep(0.1)
    ...
    def send_key(self, key_name, duration=0.1):
        try:
            m = {"end": 0x23, "space": 0x20, "alt": 0x12, "left": 0x25, "right": 0x27, "ctrl": 0x11, "shift": 0x10}
    ...
    def toggle_running(self):
        if self.is_running:
            self.is_running = False
            self.start_btn.config(text="마스터 피스 시작 (F12)", bg="#00adb5")
            self.log("중단.")
        else:
            ...
            if np.any(mask):
                ...
                if M["m00"] > 0:
                    ...
                    if cx >= self.x_max.get() and cur_dir_vk == win32con.VK_RIGHT:
                        ...
                        cur_dir_vk = win32con.VK_LEFT; self.log(">>> 전환: 좌측으로")
                        ...
                        cur_dir_vk = win32con.VK_RIGHT; self.log("<<< 전환: 우측으로")
    ...
    def delete_profile(self):
        ...
        if n in self.profiles_data: 
            del self.profiles_data[n]; self.update_profile_list()
            with open(CONFIG_FILE, "w", encoding="utf-8") as f: json.dump(self.profiles_data, f, ensure_ascii=False)
