...
class AUTOmapleV9_7(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("AUTOmaple 도형거탐버전 v9.7.1 (2026.06.02)")
    ...
    def start_hunting(self):
        if not self.is_running:
            # UI 값 캡처 (스레드 안전 확보)
            self.cur_key_att = self.key_att_cb.currentText()
            self.cur_key_dash = self.key_dash_cb.currentText()
            self.cur_key_jump = self.key_jump_cb.currentText()
            self.cur_key_pet = self.key_pet_cb.currentText()
                    ...
                    try:
                        ...
                        if np.any(mask):
                            ...
                            if M["m00"] > 0:
                                ...
                                if now_ms - l_att >= self.attack_delay_ms: self.press_key(self.cur_key_att); l_att = now_ms
                                if now_ms - l_dash >= self.dash_delay_ms: self.press_key(self.cur_key_dash); l_dash = now_ms
                                if (time.time() - l_del_time) >= (self.periodic_interval_min * 60):
                                    self.log("소모품을 자동으로 사용했습니다."); self.press_key(self.cur_key_pet); l_del_time = time.time()
                            if cx != -1:
                                if abs(cx - last_x) < 2: stuck_cnt += 1
                                else: stuck_cnt = 0; last_x = cx
                                if stuck_cnt > 40: self.press_key(self.cur_key_jump); stuck_cnt = 0
                    except Exception as e:
                        if self.err_cnt % 100 == 0: self.log(f"Core Loop Error: {e}")
                time.sleep(0.04)
        finally:
            pyautogui.keyUp('left'); pyautogui.keyUp('right')
            self.is_running = False
    ...
    def anti_macro_loop(self):
        ANTI_IMAGES = [os.path.join(BASE_DIR, f"anti{i}.png") for i in range(1, 5)]
        last_alert = 0
        with mss.mss() as sct:
            while not self.stop_threads:
                ...
                try:
                    with mss.mss() as sct:
                        ...
                        if contours:
                            ...
                            # 고강도 필터링: 면적 임계값 상향 + 종횡비 검증
                            if area > 180:
                                x, y, w, h = cv2.boundingRect(best_cnt)
                                aspect_ratio = w / float(h) if h > 0 else 0
                                
                                # 거탐 도형은 대략적인 정사각형/원형 범주 (0.5~2.0 사이)
                                if 0.4 < aspect_ratio < 2.5:
                                    detect_cnt += 1
                                    conf = min(area / 1500.0, 1.0)
                                    M = cv2.moments(best_cnt)
                                    if M["m00"] > 0:
                                        cX_local, cY_local = int(M["m10"]/M["m00"]), int(M["m01"]/M["m00"])
                                        cX_global, cY_global = cX_local + monitor["left"], cY_local + monitor["top"]
                                        
                                        cv2.rectangle(debug_feed, (x, y), (x+w, y+h), (0, 0, 255), 2)
                                        cv2.drawMarker(debug_feed, (cX_local, cY_local), (0, 255, 0), cv2.MARKER_CROSS, 20, 2)
                                        cv2.putText(debug_feed, f"TRACKING: {conf:.2f}", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

                                        # 연속 프레임 검증 (3프레임 이상 노출 시에만 실제 동작)
                                        if detect_cnt >= 3 and conf > 0.12:
                                            self.bezier_move(cX_global, cY_global)
                                            log_msg = f"f [{self.frame_num:05d}] ACTIONbezier_move tracking={conf:.2f}"
                                        else:
                                            log_msg = f"f [{self.frame_num:05d}] detecting... ({detect_cnt}/3)"
                                else:
                                    detect_cnt = 0
                            else:
                                detect_cnt = 0
                        ...
                        if not log_msg: log_msg = f"f [{self.frame_num:05d}] searching..."
                        cv2.putText(debug_feed, f"FRAME: {self.frame_num}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    ...
    def status_update_loop(self):
        while not self.stop_threads:
            try:
                ...
                total_uptime_sec = self.total_hunting_time + uptime_sec
