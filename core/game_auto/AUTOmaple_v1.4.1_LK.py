...
class AUTOmapleV9_7(QMainWindow):
    def __init__(self):
        ...
        try:
            ...
            keyboard.add_hotkey('f2', self.hotkey_record_waypoint) # F2를 순환 좌표 기록 단축키로 변경
        except: pass
    ...
    def setup_ui(self):
        ...
        self.lbl_waypoints = []
        for idx in range(6):
            lbl = QLabel(f"슬롯 {idx+1}: X: 미지정, Y: 미지정")
                        ...
                        if self.use_fishing_mode and self.fish_x != -1 and self.fish_y != -1:
                            if cx != -1 and cy != -1:
                                ...
                                if abs(dx) > 5 or abs(dy) > 5:
                                    # X축이 먼저 일치할 때까지 걷기로 정렬 (오차 ±5px 이내)
                                    if abs(dx) > 5:
                                        ...
                                        # Y축 복귀 시퀀스 작동 (목표보다 아래에 있으면 Up + Teleport)
                                        if dy > 5:
                        ...
                        if cx != -1 and cy != -1:
                            ...
                            # D. 순환사냥 V2 연산
                                ...
                                if abs(dx) <= 4 and abs(dy) <= 4:
                                    ...
                                    # 다음 순환 목표로 인덱스 변경
                                    list_pos = next(i for i, v in enumerate(valid_pts) if v[0] == target_idx)
                                    next_list_pos = (list_pos + 1) % len(valid_pts)
                                    self.current_waypoint_idx = valid_pts[next_list_pos][0]
                                    self.log(f"🗺️ 다음 목표로 이동 -> 슬롯 {self.current_waypoint_idx+1} (X:{self.waypoints[self.current_waypoint_idx][0]}, Y:{self.waypoints[self.current_waypoint_idx][1]})")
                                    time.sleep(0.1)
                                    continue
    ...
    def reset_waypoints(self):
        ...
        self.current_waypoint_idx = 0
        for idx in range(6):
            self.lbl_waypoints[idx].setText(f"슬롯 {idx+1}: X: 미지정, Y: 미지정")
