...
class AUTOmapleV9_7(QMainWindow):
    def __init__(self):
        super().__init__()
        self.version = "v1.5.1"
        self.setWindowTitle(f"AUTOmaple {self.version} (2026.06.08 - V3 고도화판)")
...
exit"""
    ...
    def hunter_core(self):
        try:
            ...
            try:
                self.return_delay = float(self.txt_pingpong_return_delay.text().strip() or "1.0")
            except Exception as e:
                self.return_delay = 1.0
                self.logger.log(f"[경고] return_delay 파싱 실패: {str(e)}", "SYSTEM")

            try:
                self.recovery_delay = float(self.txt_pingpong_recovery_delay.text().strip() or "1.0")
            except Exception as e:
                self.recovery_delay = 1.0
                self.logger.log(f"[경고] recovery_delay 파싱 실패: {str(e)}", "SYSTEM")
                    ...
                    if self.hunt_mode == 0:
                        ...
                        if self.is_falling_recovering:
                            ...
                            if self.falling_recovery_state == "moving_to_return_start":
                                ...
                                if abs(cy - target_y) > 4:
                                    ...
                                    self.set_key_state('right', False)
                                    dy = cy - target_y
                                    use_tele = getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked()
                                    move_mode = "TELEPORT" if use_tele else "JUMP"
                                ...
                                if self.teleport_x != -1 and self.teleport_y != -1:
                                    ...
                                    if abs(cx - self.teleport_x) <= 1:
                                        ...
                                        if cx < self.teleport_x:
                                            self.set_key_state('left', False)
                                            self.set_key_state('right', True)
                                            if getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked() and self.teleport_x - cx > 25:
                                            ...
                                            self.set_key_state('right', False)
                                            self.set_key_state('left', True)
                                            if getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked() and cx - self.teleport_x > 25:
                                                self.press_key(self.key_teleport_cb.currentText())
                                                time.sleep(0.15)
                                            else:
                                                time.sleep(0.05)
                                    ...
                                    if self.return_x_min <= cx <= self.return_x_max:
                                        ...
                                        if cx < self.return_x_min:
                                            self.set_key_state('left', False)
                                            self.set_key_state('right', True)
                                            if getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked() and self.return_x_min - cx > 25:
                                                self.press_key(self.key_teleport_cb.currentText())
                                                time.sleep(0.15)
                                            else:
                                                time.sleep(0.05)
                                            ...
                                            self.set_key_state('right', False)
                                            self.set_key_state('left', True)
                                            if getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked() and cx - self.return_x_max > 25:
                                                self.press_key(self.key_teleport_cb.currentText())
                                                time.sleep(0.15)
                                            else:
                                                time.sleep(0.05)
                                ...
                                if seq_str.upper() == "AUTO":
                                    # 대안 B: 자동 목적지 이동 (Pathfinding)
                                    target_y = self.teleport_y if (self.teleport_x != -1 and self.teleport_y != -1) else self.hunt_y
                                    target_x = self.teleport_x if (self.teleport_x != -1 and self.teleport_y != -1) else int((self.return_x_min + self.return_x_max) / 2)
                                    
                                    self.logger.log(f"[자동복귀] 목적지 X:{target_x}, Y:{target_y} 경로 자동 탐색 시작", "MOVEMENT")
                                    auto_start_time = time.time()
                                    while self.is_running and not self.is_selling and not self.is_dragging_anti:
                                        with mss.mss() as sct:
                                            cx, cy, _, _ = self.minimap_detector.get_player_coords(sct)
                                        
                                        if cx == -1 or cy == -1:
                                            time.sleep(0.05)
                                            continue
                                            
                                        if abs(cy - target_y) <= 3 and (self.return_x_min <= cx <= self.return_x_max or abs(cx - target_x) <= 4):
                                            self.logger.log("[자동복귀] 목표 지점 도달 완료", "MOVEMENT")
                                            break
                                            
                                        if time.time() - auto_start_time > 10.0:
                                            self.logger.log("[자동복귀] 도달 시간 초과로 강제 완료 처리", "MOVEMENT")
                                            break
                                            
                                        if abs(cy - target_y) > 3:
                                            self.set_key_state('left', False)
                                            self.set_key_state('right', False)
                                            dy = cy - target_y
                                            use_tele = getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked()
                                            ...
                                            if abs(dx) > 3:
                                                ...
                                                self.set_key_state('left' if move_dir == 'right' else 'right', False)
                                                self.set_key_state(move_dir, True)
                                                if getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked() and abs(dx) > 15:
                                                    self.press_key(self.key_teleport_cb.currentText())
                                                    time.sleep(0.15)
                                                else:
                                                    time.sleep(0.05)
                                                self.set_key_state(move_dir, False)
                                            else:
                                                time.sleep(0.05)
                                elif seq_str:
                                    ...
                                    for idx, act in enumerate(actions):
                                        ...
                                        if ":" in act:
                                            try:
                                                parts = act.split(":")
                                                act_delay = float(parts[-1].strip())
                                            except Exception as e:
                                                self.logger.log(f"[시퀀스 경고] 딜레이 파싱 실패 ({act}): {str(e)}", "SYSTEM")
                        ...
                        if self.use_recovery:
                            ...
                            if not self.is_recovering and (time.time() - self.last_recovery_time > self.recovery_interval):
                                self.is_recovering = True
                                self.recovery_state = "moving_to_recovery_start"
                                self.reached_recovery_start = False
                                self.logger.log(f"[핑퐁회수] 회수 주기 도달 -> 회수 전 지정 시작좌표로 이동 시도", "MOVEMENT")
                                self.set_key_state('left', False)
                                self.set_key_state('right', False)
                            ...
                            if self.is_recovering:
                                ...
                                if self.recovery_state == "moving_to_recovery_start":
                                    # 실시간 시작좌표 파싱
                                    try:
                                        self.recovery_start_x = int(self.txt_pingpong_recovery_start_x.text().strip() or "-1")
                                    except:
                                        self.recovery_start_x = -1
                                    try:
                                        self.recovery_start_y = int(self.txt_pingpong_recovery_start_y.text().strip() or "-1")
                                    except:
                                        self.recovery_start_y = -1
                                    
                                    # 시작좌표가 지정되어 있지 않거나 이미 도달한 경우 즉시 회수층 이동으로 전환
                                    if self.recovery_start_x == -1 or self.recovery_start_y == -1 or getattr(self, 'reached_recovery_start', False):
                                        self.recovery_state = "moving_to_recovery_y"
                                        self.reached_recovery_start = True
                                        self.logger.log(f"[핑퐁회수] 지정 시작좌표 미설정 또는 이미 도달 -> 회수층(Y: {self.recovery_y}) 이동 시작", "MOVEMENT")
                                        continue
                                        
                                    target_y = self.recovery_start_y
                                    
                                    # Y축 정렬
                                    if abs(cy - target_y) > 4:
                                        self.logger.log(f"[핑퐁회수] 지정 시작좌표로 Y 정렬 시도 (현재 Y: {cy}, 목표 Y: {target_y})", "MOVEMENT")
                                        self.set_key_state('left', False)
                                        self.set_key_state('right', False)
                                        dy = cy - target_y
                                        # 지정 시작좌표 이동시는 무조건 텔레포트(상승)/하향점프(하강)를 지원하여 고속 정렬
                                        move_mode = "TELEPORT" if dy > 0 else "JUMP"
                                        self.movement_controller.perform_vertical_move(dy, move_mode)
                                        time.sleep(0.25)
                                        continue
                                        
                                    # X축 정렬
                                    self.logger.log(f"[핑퐁회수] 지정 시작좌표 이동 중 : X={cx} Y={cy} (목표 X={self.recovery_start_x})", "MOVEMENT")
                                    if abs(cx - self.recovery_start_x) <= 1:
                                        self.set_key_state('left', False)
                                        self.set_key_state('right', False)
                                        self.reached_recovery_start = True
                                        self.recovery_state = "moving_to_recovery_y"
                                        self.logger.log(f"[핑퐁회수] 지정 시작좌표 도달 완료 -> 회수층(Y: {self.recovery_y}) 이동 시작", "MOVEMENT")
                                    else:
                                        if cx < self.recovery_start_x:
                                            self.set_key_state('left', False)
                                            self.set_key_state('right', True)
                                            if self.recovery_start_x - cx > 15:
                                                self.press_key(self.key_teleport_cb.currentText())
                                                time.sleep(0.15)
                                            else:
                                                time.sleep(0.05)
                                            ...
                                            self.set_key_state('right', False)
                                            self.set_key_state('left', True)
                                            if cx - self.recovery_start_x > 15:
                                                self.press_key(self.key_teleport_cb.currentText())
                                                time.sleep(0.15)
                                            else:
                                                time.sleep(0.05)
                                ...
                                if self.recovery_state == "moving_to_recovery_y":
                                    ...
                                    if time.time() - self.recovery_start_time > self.recovery_duration:
                                        ...
                                        # 핑퐁 회수 시 텔레포트 적용 (체크박스 활성화 시에만 작동)
                                        if getattr(self, 'chk_pingpong_recovery_teleport', None) and self.chk_pingpong_recovery_teleport.isChecked():
                                    ...
                                    # 지정한 좌표로 이동해야 하는 경우 우선 처리
                                    if self.recovery_start_x != -1 and self.recovery_start_y != -1 and not getattr(self, 'reached_recovery_start', False):
                                        target_y = self.recovery_start_y
                                        ...
                                        if abs(cx - self.recovery_start_x) <= 1:
                                            ...
                                            self.set_key_state('right', False)
                                            self.reached_recovery_start = True
                                            self.logger.log("[핑퐁회수] 지정 시작좌표 도달 완료 -> 텔포포인트로 이동합니다.", "MOVEMENT")
                                        else:
                                            if cx < self.recovery_start_x:
                                                self.set_key_state('left', False)
                                                self.set_key_state('right', True)
                                    ...
                                    if self.teleport_x != -1 and self.teleport_y != -1:
                                        # 1순위: 텔포포인트(T) 이동 (이동 스킬은 사용하지 않고 오직 걸어서!)
                                        self.logger.log(f"[핑퐁회수]\n현재 위치 : X={cx} Y={cy} (목표 텔포포인트 X={self.teleport_x})", "MOVEMENT")
                                        if abs(cx - self.teleport_x) <= 1:
                                            self.set_key_state('left', False)
                                            self.set_key_state('right', False)
                                        ...
                                        # 2순위: 복귀 X범위 이동 (텔포포인트 지정이 없는 경우에도 걸어서 안전하게!)
                                        if self.return_x_min <= cx <= self.return_x_max:
                                            self.set_key_state('left', False)
                                            self.set_key_state('right', False)
                                            self.recovery_state = "executing_return_seq"
                        ...
                        if getattr(self, 'chk_pingpong_fixed', None) and self.chk_pingpong_fixed.isChecked():
                            ...
                            # Y 좌표 인지 기반 공격 방지 로직: 현재 회수층 부근이거나 사냥층을 벗어난 상태인 경우 스킬 사용 불가능하게 해제
                            is_at_hunt_layer = (cy == -1 or abs(cy - self.hunt_y) <= 3)
                            is_at_recovery_layer = (cy != -1 and abs(cy - self.recovery_y) <= 3)
    ...
    def execute_sequence_action(self, act):
        ...
        if "+" in act:
            ...
            for sa in sub_acts:
                sa_name = sa.split(":")[0].strip()
                self.execute_single_sequence_action(sa_name)
        else:
            act_name = act.split(":")[0].strip()
            self.execute_single_sequence_action(act_name)
    ...
    def execute_single_sequence_action(self, act):
        ...
        if act == "TELE_UP":
            ...
            if getattr(self, 'is_recovering', False):
                self.logger.log(f"[회수모드] 공격 스킬 입력 무시 ({att_key})", "MOVEMENT")
            else:
    ...
    def status_update_loop(self):
        while not self.stop_threads:
            try:
                ...
                pet_left_str = "비활성"
                if self.is_running:
                    last_pet = getattr(self, 'last_pet_buff_time', 0.0)
                    pet_interval_sec = self.periodic_interval_min * 60
                    time_left_pet = max(0, int(pet_interval_sec - (time.time() - last_pet)))
                    pet_left_str = f"{time_left_pet}초"
