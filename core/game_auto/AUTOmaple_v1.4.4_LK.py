...
class AUTOmapleV9_7(QMainWindow):
    def __init__(self):
        super().__init__()
        self.version = "v1.4.4"
        self.setWindowTitle(f"AUTOmaple {self.version} (2026.06.05)")
        ...
        # 순환사냥 V2 변수
        self.waypoints = [(-1, -1)] * 6
        self.visited_waypoints = [False] * 6
        ...
        try:
            ...
            keyboard.add_hotkey('f2', self.hotkey_record_waypoint)
        except: pass
        
        # 초기 사냥 모드 상태에 따른 라디오 버튼 및 UI 활성 상태 설정
        if self.hunt_mode == 0:
            self.radio_lr.setChecked(True)
        elif self.hunt_mode == 1:
            self.radio_stat.setChecked(True)
    ...
    def setup_ui(self):
        ...
        self.radio_lr = QRadioButton("좌우 이동")
        self.radio_stat = QRadioButton("제자리 사냥")
        self.radio_v2 = QRadioButton("순환사냥 V2")
        
        # UI 스타일
        for r_btn in [self.radio_lr, self.radio_stat, self.radio_v2]:
        ...
        self.btn_group_mode.addButton(self.radio_lr, 0)
        self.btn_group_mode.addButton(self.radio_stat, 1)
        self.btn_group_mode.addButton(self.radio_v2, 2)
        
        mode_select_layout.addWidget(self.radio_lr)
        mode_select_layout.addWidget(self.radio_stat)
        mode_select_layout.addWidget(self.radio_v2)
        tab_hunt_mode_vbox.addWidget(mode_select_box)
        
        # 2. 모드별 세부 설정 그룹
        # A. 좌우 이동 설정 그룹
        self.grp_lr_settings = QGroupBox("좌우 이동 설정")
        lr_layout = QVBoxLayout(self.grp_lr_settings)
        self.x_min_slider = self.create_slider_row(lr_layout, "좌측 경계:", 0, 400, self.x_min, self.update_x_min)
        self.x_max_slider = self.create_slider_row(lr_layout, "우측 경계:", 0, 400, self.x_max, self.update_x_max)
        tab_hunt_mode_vbox.addWidget(self.grp_lr_settings)
        
        # B. 제자리 사냥 설정 그룹
        self.grp_stat_settings = QGroupBox("제자리 사냥 설정")
        stat_layout = QVBoxLayout(self.grp_stat_settings)
        self.stat_range_slider = self.create_slider_row(stat_layout, "제자리 범위:", 1, 100, self.stationary_range, self.update_stat_range)
        tab_hunt_mode_vbox.addWidget(self.grp_stat_settings)
        
        # C. 순환사냥 V2 설정 그룹
        self.grp_v2_settings = QGroupBox("순환사냥 V2 설정")
        v2_layout = QVBoxLayout(self.grp_v2_settings)
        v2_layout.addWidget(QLabel("📍 순환 좌표 기록 슬롯 (F2 등록 및 수동 입력)", objectName="subLabel"))
        
        self.lbl_waypoints = []
        self.txt_waypoints_x = []
        self.txt_waypoints_y = []
        
        for idx in range(6):
            row = QHBoxLayout()
            row.setSpacing(8)
            
            lbl = QLabel(f"슬롯 {idx+1}:")
            lbl.setStyleSheet("color: #8a99af; font-weight: bold; font-size: 13px;")
            lbl.setFixedWidth(50)
            self.lbl_waypoints.append(lbl)
            row.addWidget(lbl)
            
            row.addWidget(QLabel("X:"))
            txt_x = QLineEdit()
            txt_x.setFixedWidth(55)
            txt_x.setFixedHeight(26)
            txt_x.setStyleSheet("background-color: #0d1117; color: #00d2ff; border: 1px solid #30363d; border-radius: 5px; font-weight: bold; font-size: 11px;")
            from PySide6.QtGui import QIntValidator
            txt_x.setValidator(QIntValidator(-1, 9999))
            txt_x.textChanged.connect(lambda text, i=idx: self.on_waypoint_txt_changed(i, 'x', text))
            self.txt_waypoints_x.append(txt_x)
            row.addWidget(txt_x)
            
            row.addWidget(QLabel("Y:"))
            txt_y = QLineEdit()
            txt_y.setFixedWidth(55)
            txt_y.setFixedHeight(26)
            txt_y.setStyleSheet("background-color: #0d1117; color: #00d2ff; border: 1px solid #30363d; border-radius: 5px; font-weight: bold; font-size: 11px;")
            txt_y.setValidator(QIntValidator(-1, 9999))
            txt_y.textChanged.connect(lambda text, i=idx: self.on_waypoint_txt_changed(i, 'y', text))
            self.txt_waypoints_y.append(txt_y)
            row.addWidget(txt_y)
            ...
            del_btn.clicked.connect(lambda checked=False, i=idx: self.delete_waypoint(i))
            ...
            up_btn.clicked.connect(lambda checked=False, i=idx: self.shift_waypoint(i, -1))
            ...
            dn_btn.clicked.connect(lambda checked=False, i=idx: self.shift_waypoint(i, 1))
            row.addWidget(dn_btn)
            
            v2_layout.addLayout(row)
            
        self.reset_waypoints_btn = QPushButton("순환 좌표 전체 초기화")
        self.reset_waypoints_btn.setFixedHeight(35)
        self.reset_waypoints_btn.clicked.connect(self.reset_waypoints)
        v2_layout.addWidget(self.reset_waypoints_btn)
        
        v2_layout.addSpacing(10)
        
        # 텔레포트 임계값 조절 슬라이더 추가
        self.tele_thresh_slider = self.create_slider_row(
            v2_layout, 
            "텔레포트 이동 임계값:", 
            15, 30, 
            self.teleport_threshold, 
            self.update_teleport_threshold
        )
        
        tab_hunt_mode_vbox.addWidget(self.grp_v2_settings)
        tab_hunt_mode_vbox.addStretch()
        
        self.main_tabs.addTab(tab_hunt_mode_widget, "사냥 모드 설정")
        
        # 라디오 시그널 연결
        self.btn_group_mode.buttonClicked.connect(self.on_hunt_mode_radio_changed)
        
        tab_skill_widget = QWidget(); tab_skill_vbox = QVBoxLayout(tab_skill_widget); self.precision_slider = self.create_slider_row(tab_skill_vbox, "인식 정밀도:", 0, 30, 0, self.update_precision, is_float=True); self.precision_slider.setEnabled(False); self.att_slider = self.create_slider_row(tab_skill_vbox, "공격 주기:", 100, 3000, self.attack_delay_ms, self.update_att_delay); self.dash_slider = self.create_slider_row(tab_skill_vbox, "텔레포트 주기:", 100, 10000, self.dash_delay_ms, self.update_dash_delay); self.pet_slider = self.create_slider_row(tab_skill_vbox, "소모품(분):", 1, 60, self.periodic_interval_min, self.update_pet_interval)
        
        key_grid = QGridLayout(); key_grid.setSpacing(12)
        self.key_att_cb = self.create_key_combo(key_grid, "공격", 0, 0, "end")
        self.key_jump_cb = self.create_key_combo(key_grid, "점프", 0, 1, "alt")
        self.key_teleport_cb = self.create_key_combo(key_grid, "텔레포트", 1, 0, "shift")
        ...
        # 복층 사냥 설정 UI 추가
        self.chk_multifloor = QCheckBox("복층 순환사냥 활성화 (방향전환 시 텔레포트 상승)")
        self.chk_multifloor.setChecked(False)
        self.chk_multifloor.toggled.connect(self.update_use_multifloor)
        tab_coord_vbox.addWidget(self.chk_multifloor)
        
        self.multifloor_up_slider = self.create_slider_row(tab_coord_vbox, "텔레포트 상승 횟수:", 0, 3, self.multifloor_up_count, self.update_multifloor_up_count)
    ...
    def hunter_core(self):
        ...
        # V2 기동 시 초기 정보 상세 로깅
        if self.hunt_mode == 2:
            valid_pts = [(idx, pt) for idx, pt in enumerate(self.waypoints) if pt[0] != -1 and pt[1] != -1]
            tx, ty = self.waypoints[self.current_waypoint_idx] if len(self.waypoints) > self.current_waypoint_idx else (-1, -1)
            self.log(f"🧭 [V2 진입 검증] 모드: {self.hunt_mode}, 유효 waypoint 개수: {len(valid_pts)}, 현재 인덱스: {self.current_waypoint_idx}, 목표 좌표: ({tx}, {ty}), 현재 캐릭터 좌표: ({self.char_x}, {self.char_y})")
            ...
            while self.is_running:
                ...
                with mss.mss() as sct:
                     ...
                     try:
                        ...
                        if self.use_fishing_mode and self.fish_x != -1 and self.fish_y != -1:
                            if cx != -1 and cy != -1:
                                ...
                                if abs(dx) > 5 or abs(dy) > 5:
                                    ...
                                    jump_key = self.key_jump_cb.currentText()
                                    
                                    # 1. X축 우선 정렬
                                    if abs(dx) > 5:
                                        if abs(dx) >= 20:
                                            # 간극 20 이상 -> 텔레포트 고속 이동 (방향 전환 확실히 선행)
                                            if dx > 0:
                                                # 목표가 왼쪽 -> 왼쪽 텔레포트
                                                self.log(f"🎣 낚시 자리 이탈 (X 오차: {dx}px) -> 왼쪽 텔레포트 고속 이동")
                                                self.set_key_state('right', False)
                                                self.set_key_state('left', True)
                                                time.sleep(0.05)
                                                ...
                                                # 목표가 오른쪽 -> 오른쪽 텔레포트
                                                self.log(f"🎣 낚시 자리 이탈 (X 오차: {dx}px) -> 오른쪽 텔레포트 고속 이동")
                                                self.set_key_state('left', False)
                                                self.set_key_state('right', True)
                                                time.sleep(0.05)
                                                self.press_key(teleport_key)
                                            ...
                                            # 간극 20 미만 -> 정교한 걷기 정렬
                                            self.log(f"🎣 낚시 자리 이탈 (X 오차: {dx}px) -> X축 우선 걷기 정렬")
                                            if dx > 0:
                                                self.set_key_state('right', False)
                                                self.set_key_state('left', True)
                                            else:
                                        ...
                                        # X축이 정렬 완료되었으므로 걷기 키 해제
                                        self.set_key_state('left', False)
                                        self.set_key_state('right', False)
                                        
                                        # 2. Y축 복귀 시퀀스 작동
                                        # 목표보다 아래에 있으면 (Y축 상승 텔레포트)
                                        if dy > 5:
                                            self.log(f"🎣 낚시 자리 Y축 이탈 (Y 오차: {dy}px) -> 텔레포트 상승")
                                            ...
                                            time.sleep(0.1)
                                            continue
                                        # 목표보다 위에 있으면 (하단 낙하 기동)
                                        elif dy < -5:
                                            self.log(f"🎣 낚시 자리 Y축 이탈 (Y 오차: {-dy}px) -> 하단 낙하 기동")
                        ...
                        # 4. 자동 복층/핑퐁/순환 사냥 연산
                        if cx != -1 and cy != -1:
                            # A. 추락 복귀 핸들링 (±1px 오차 허용)
                            if self.use_fall_recovery and cy >= (self.fall_y_threshold - 1):
                                if not self.is_falling_recovering:
                                    self.log(f"🚨 캐릭터 추락 감지 (Y: {cy} >= 임계값: {self.fall_y_threshold - 1}). 텔레포트 복귀 동작 개시!")
                            ...
                            if self.hunt_mode == 0:
                                ...
                                if reached_boundary and self.use_multifloor_hunt and self.multifloor_up_count > 0:
                                    self.log(f"🪜 복층 이동 기동: 방향전환 시 텔레포트 상승 {self.multifloor_up_count}회 수행")
                                    teleport_key = self.key_teleport_cb.currentText()
                                    
                                    # up 키 누른 채 텔레포트 입력
                                    self.set_key_state('up', True)
                                    time.sleep(0.08)
                                    for _ in range(self.multifloor_up_count):
                            ...
                            # D. 순환사냥 V2 연산
                                ...
                                # 단계별 로그 추가: 순환사냥 V2 진입 확인
                                self.log(f"🧭 [V2 루틴 실행] 캐릭터 위치: ({cx}, {cy}), 유효 waypoint 개수: {len(valid_pts)}, 현재 목표 인덱스: {self.current_waypoint_idx}")
                                
                                if not valid_pts:
                                    self.log("⚠️ 순환사냥 V2 경고: 기록된 유효한 좌표가 없습니다. 좌우 이동으로 사냥합니다.")
                                    self.hunt_mode = 0
                                    time.sleep(0.2)
                                    continue

                                valid_indices = [v[0] for v in valid_pts] # 예: [0, 2, 4]

                                target_idx = self.current_waypoint_idx
                                # 현재 타겟 인덱스가 유효 좌표 리스트에 없으면, 유효 좌표 중 오름차순 첫 번째 좌표를 타겟팅
                                if target_idx not in valid_indices:
                                    target_idx = valid_indices[0]
                                    self.current_waypoint_idx = target_idx
                                    self.log(f"🗺️ [V2 고정순환] 오름차순 목적지 재설정 -> 슬롯 {target_idx+1} (X: {self.waypoints[target_idx][0]}, Y: {self.waypoints[target_idx][1]})")

                                tx, ty = self.waypoints[target_idx]
                                dx = cx - tx
                                dy = cy - ty

                                # 단계별 로그 추가: 목표 및 오차 로깅
                                self.log(f"🗺️ [V2 목표 추적] 슬롯 {target_idx+1} -> 목표: ({tx}, {ty}), 현재 캐릭터: ({cx}, {cy}), 오차: DX={dx}, DY={dy}")

                                # 목표 좌표 안착 판정 (X 오차 ±5px, Y 오차 ±5px 이내)
                                if abs(dx) <= 5 and abs(dy) <= 5:
                                    if self.landing_start_time is None:
                                        self.landing_start_time = time.time()

                                    # 0.3초 이상 안정적으로 유지되었는지 확인
                                    if time.time() - self.landing_start_time >= 0.3:
                                        # 1. 방향키 완전 해제 보장
                                        self.set_key_state('left', False)
                                        self.set_key_state('right', False)
                                        self.set_key_state('up', False)
                                        ...
                                        self.is_moving = False
                                        self.is_teleporting = False
                                        time.sleep(0.05) # 키 해제 완료를 위한 미세 대기

                                        self.is_attacking = True
                                        self.log(f"🎯 순환 좌표 슬롯 {target_idx+1} 안착 완료 (0.3초 대기 검증 완료) -> 공격 개시")

                                        # 0.5초 동안 제자리 스킬 난사
                                        att_start = time.time()
                                        while time.time() - att_start < 0.5 and self.is_running:
                                            now = time.time()*1000
                                            if now - l_att >= self.attack_delay_ms:
                                                self.log(f"⚔️ [V2 안착 공격] 설정: {self.attack_delay_ms}ms, 실제 지연: {int(now - l_att)}ms (차이: {int(now - l_att) - self.attack_delay_ms}ms)")
                                                att_key = self.key_att_cb.currentText()
                                                # 공격키 입력 → 0.03~0.05초 대기 → 공격키 해제
                                        ...
                                        self.log(f"🗺️ 다음 오름차순 목적지 갱신 -> 슬롯 {self.current_waypoint_idx+1} (X: {self.waypoints[self.current_waypoint_idx][0]}, Y: {self.waypoints[self.current_waypoint_idx][1]})")
                                        time.sleep(0.1)
                                        continue
                                else:
                                    # 안착 범위를 벗어났으면 안착 타이머 초기화
                                    self.landing_start_time = None
                                ...
                                if abs(dx) > 5:
                                    if abs(dx) >= threshold:
                                        ...
                                        # 방향키 다운 -> 0.04초 유지 -> 텔레포트 -> 0.02초 추가 유지 -> 방향키 업
                                        ...
                                        time.sleep(0.15) # 텔레포트 후 딜레이
                                        self.is_teleporting = False
                                    else:
                                        self.is_moving = True
                                        # 간극이 임계값 미만 -> 방향키로 걷기 이동
                                        if dx > 0:
                                            self.set_key_state('right', False)
                                            self.set_key_state('left', True)
                                        else:
                                            ...
                                            self.set_key_state('right', True)
                                        time.sleep(0.05)
                                    continue # X축을 맞추는 동안에는 Y축 수직이동을 보류

                                # 2. X축이 거의 맞으면 (abs(dx) <= 5), Y축 상승/낙하 기동 수행
                                else:
                                    self.set_key_state('left', False)
                                    self.set_key_state('right', False)
                                    self.is_moving = False

                                    # 수직 복귀 실패 감지를 위해 동작 전 Y 좌표 기록
                                    self.last_char_y = cy

                                    # Y축 수직 상승 기동 (나보다 위에 있고 5px 초과)
                                    if dy > 5:
                                        self.is_teleporting = True
                                        self.log(f"🪜 수직 상승 이동 기동 (Y차이: {dy}px, 실패 카운트: {self.fall_count}/4)")
                                        self.set_key_state('up', True)
                                        time.sleep(0.04)  # 방향 선입력과 동일하게 up 선입력 0.04초
                                        self.press_key(teleport_key)
                                        time.sleep(0.02)  # up키 0.02초 유지
                                        self.set_key_state('up', False)
                                        time.sleep(0.2)
                                        self.is_teleporting = False

                                        # 상승 기동 후 Y좌표가 유의미하게 달라졌는지 확인
                                        time.sleep(0.05)

                                    # Y축 하단 낙하 기동 (나보다 아래에 있고 5px 초과)
                                    elif dy < -5:
                                        self.is_moving = True
                                        self.log(f"⬇️ 하단 낙하 기동 (Y차이: {-dy}px, 실패 카운트: {self.fall_count}/4)")
                                        jump_key = self.key_jump_cb.currentText()
                                        self.set_key_state('down', True)
                                        time.sleep(0.04)  # down 선입력 0.04초
                                        self.press_key(jump_key)
                                        time.sleep(0.02)  # down키 0.02초 유지
                                        self.set_key_state('down', False)
                                        time.sleep(0.2)
                                        self.is_moving = False
                                    ...
                                    # 동작 후 Y좌표 스캔을 위해 화면 다시 캡처 및 좌표 탐색
                                    with mss.mss() as sct_check:
                                        reg_check = {"top": self.reg_t, "left": self.reg_l, "width": self.reg_w, "height": self.reg_h}
                                        try:
                                            shot_check = np.array(sct_check.grab(reg_check))
                                            img_check = cv2.cvtColor(shot_check, cv2.COLOR_BGRA2BGR)
                                            m_check = self.color_margin
                                            low_check = np.array([max(0, self.base_lower[2]-m_check), max(0, self.base_lower[1]-m_check), max(0, self.base_lower[0]-m_check)])
                                            high_check = np.array([min(255, self.base_upper[2]+m_check), min(255, self.base_upper[1]+m_check), min(255, self.base_upper[0]+m_check)])
                                            mask_check = cv2.inRange(img_check, low_check, high_check)
                                            if np.any(mask_check):
                                                M_check = cv2.moments(mask_check)
                                                if M_check["m00"] > 0:
                                                    new_cy = int(M_check["m01"]/M_check["m00"])
                                                    if abs(new_cy - self.last_char_y) <= 2:
                                                        self.fall_count += 1
                                                    else:
                                                        self.fall_count = 0
                                        except:
                                ...
                                if self.fall_count >= 4:
                                    self.log(f"⚠️ [수직 복귀 실패 감지] 연속 {self.fall_count}회 수직 이동 실패. 목적지 강제 전환 및 비상 탈출을 실행합니다.")
                                    self.fall_count = 0
                                    ...
                                    self.landing_start_time = None
                                    self.log(f"🗺️ [V2 고정순환] 수직 이동 실패로 인한 다음 목적지 강제 갱신 -> 슬롯 {self.current_waypoint_idx+1}")
                                    continue
                             
                            now = time.time()*1000
                            # 핑퐁 사냥(mode==0) 및 제자리 사냥(mode==1) 공격/딜레이 연산 통합
                            if self.hunt_mode == 0 or self.hunt_mode == 1:
                                if now - l_att >= self.attack_delay_ms:
                                    self.log(f"⚔️ [공격 실행] 설정: {self.attack_delay_ms}ms, 실제 지연: {int(now - l_att)}ms (차이: {int(now - l_att) - self.attack_delay_ms}ms)")
                                    self.press_key(self.key_att_cb.currentText())
                                    l_att = now
                                if self.hunt_mode == 0 and now - l_dash >= self.dash_delay_ms:
                                    self.log(f"⚡ [텔레포트 실행] 설정: {self.dash_delay_ms}ms, 실제 지연: {int(now - l_dash)}ms (차이: {int(now - l_dash) - self.dash_delay_ms}ms)")
                                    self.press_key(self.key_teleport_cb.currentText())
                                    l_dash = now
                            
                            # 펫 소모품 버프 실행 단순화 (모든 모드 공통 주기 실행)
                            if time.time() - l_del >= self.periodic_interval_min*60:
                                self.press_key(self.key_pet_cb.currentText())
                                l_del = time.time()
            ...
            while not self.stop_threads:
                if self.reg_w > 5:
                    ...
                    try:
                        ...
                        # 순환 사냥 V2 좌표들 시각화 (인덱스 숫자로 표기 및 보라색 원형 렌더링)
                        for idx, pt in enumerate(self.waypoints):
                            wx, wy = pt
                            if wx != -1 and wy != -1:
                                cv2.circle(p_img, (wx, wy), 5, (168, 85, 247), 1)
                                cv2.putText(p_img, str(idx+1), (wx - 3, wy + 4), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (168, 85, 247), 1)
    ...
    def auto_detect_minimap(self):
        ...
        try:
            with mss.mss() as sct:
                ...
                # 인식 정밀도 0 고정과 무관하게 캐릭터 노란색 점은 항상 마진 60 수준으로 든든하게 탐색
                m = 60
                low = np.array([max(0, self.base_lower[2]-m), max(0, self.base_lower[1]-m), max(0, self.base_lower[0]-m)])
                high = np.array([min(255, self.base_upper[2]+m), min(255, self.base_upper[1]+m), min(255, self.base_upper[0]+m)])
                mask = cv2.inRange(img, low, high)
                ...
                for c in contours:
                    x, y, w, h = cv2.boundingRect(c)
                    # 1280x720 해상도에서 실제 지형 캔버스의 타겟 규격 완화 (120px~280px, 60px~180px)
                ...
                # 만약 에지 팽창으로 실패하면, 다중 이진화 임계값(75, 105, 135) 순차 적용하여 검출 시도
                if best_rect is None:
                    for thresh_val in [75, 105, 135]:
                        ...
                        if candidate_rects:
                            best_rect = min(candidate_rects, key=lambda r: r[2] * r[3])
                            self.log(f"💡 다중 임계값 탐색 성공 (임계치: {thresh_val})")
                            break
                            
                # 만약 이진화로도 실패하면, 가변 미니맵 크기 대응을 위한 광선 추적 경계 탐색(Ray Casting) 실행
                if best_rect is None:
                    h_limit, w_limit = gray.shape
                    
                    # 1. 좌측 경계 탐색
                    left_x = 0
                    consecutive = 0
                    for x in range(cx, 0, -1):
                        if gray[cy, x] > 140:  # 미니맵 창의 밝은 하늘색 테두리 검출
                            consecutive += 1
                            if consecutive >= 3:
                                left_x = x + 2
                                break
                            ...
                            consecutive = 0
                    if left_x == 0: left_x = max(0, cx - 100)
                    
                    # 2. 우측 경계 탐색
                    right_x = w_limit - 1
                    consecutive = 0
                    for x in range(cx, w_limit):
                        if gray[cy, x] > 140:
                            consecutive += 1
                            if consecutive >= 3:
                                right_x = x - 2
                            ...
                            consecutive = 0
                    if right_x == w_limit - 1: right_x = min(w_limit - 1, cx + 100)
                    
                    # 3. 상단 경계 탐색 (맵이름 타이틀바가 있는 하늘색 영역 검출)
                    top_y = 0
                    consecutive = 0
                    for y in range(cy, 0, -1):
                        if gray[y, cx] > 140:
                            ...
                            consecutive = 0
                    if top_y == 0: top_y = max(0, cy - 75)
                    
                    # 4. 하단 경계 탐색
                    bottom_y = h_limit - 1
                    consecutive = 0
                    for y in range(cy, h_limit):
                        if gray[y, cx] > 140:
                            consecutive += 1
                            if consecutive >= 3:
                                bottom_y = y - 2
                            ...
                            consecutive = 0
                    if bottom_y == h_limit - 1: bottom_y = min(h_limit - 1, cy + 75)
                    
                    det_w = right_x - left_x
                    det_h = bottom_y - top_y
                    
                    # 가변 크기가 사양 한계 내에 있는지 검증 후 확정
                    if 80 < det_w < 320 and 40 < det_h < 200:
                        best_rect = (left_x, top_y, det_w, det_h)
                        self.log("🎯 광선 추적 기반 동적 가변 경계 락온 완료!")
    ...
    def start_hunting(self): 
        ...
        # 사냥 시작 검증 로그 추가
        self.log(f"🔍 [시작 검증] is_moving: {self.is_moving}, is_running(is_hunting): {self.is_running}, hunt_mode: {self.hunt_mode}")
        
        # 사냥 코어 스레드 시작
        self.hunter_thread = threading.Thread(target=self.hunter_core, daemon=True)
        self.hunter_thread.start()
        self.log("🚀 사냥 시스템을 가동합니다.")
    ...
    def update_waypoint_ui(self, idx):
        self.txt_waypoints_x[idx].blockSignals(True)
        self.txt_waypoints_y[idx].blockSignals(True)
        
        wx, wy = self.waypoints[idx]
        self.txt_waypoints_x[idx].setText(str(wx) if wx != -1 else "")
        self.txt_waypoints_y[idx].setText(str(wy) if wy != -1 else "")
        
        self.txt_waypoints_x[idx].blockSignals(False)
        self.txt_waypoints_y[idx].blockSignals(False)

    def on_waypoint_txt_changed(self, idx, axis, text):
        try:
            val = int(text) if text.strip() != "" else -1
        except:
            val = -1
        
        wx, wy = self.waypoints[idx]
        if axis == 'x':
            self.waypoints[idx] = (val, wy)
        else:
            self.waypoints[idx] = (wx, val)
        self.log(f"✏️ [수동 입력 좌표 반영] 슬롯 {idx+1} -> 축: {axis.upper()}, 값: {val}, 반영 후 전체: {self.waypoints[idx]}")

    def delete_waypoint(self, idx):
        self.waypoints[idx] = (-1, -1)
        self.update_waypoint_ui(idx)
        self.log(f"🧹 순환 좌표 슬롯 {idx+1} 초기화 완료")
        winsound.Beep(1000, 150)

    def shift_waypoint(self, idx, direction):
        target = idx + direction
        if 0 <= target < 6:
            self.waypoints[idx], self.waypoints[target] = self.waypoints[target], self.waypoints[idx]
            self.update_waypoint_ui(idx)
            self.update_waypoint_ui(target)
            self.log(f"🔄 순환 좌표 슬롯 {idx+1} <=> 슬롯 {target+1} 위치 교환 완료")
            winsound.Beep(1100, 150)

    def update_teleport_threshold(self, v):
        self.teleport_threshold = v

    def hotkey_record_waypoint(self):
        if self.char_x != -1 and self.char_y != -1:
            idx = self.waypoint_record_idx
            self.waypoints[idx] = (self.char_x, self.char_y)
            self.update_waypoint_ui(idx)
            self.log(f"📍 [순환 좌표 기록] 슬롯 {idx+1} -> X: {self.char_x}, Y: {self.char_y}")
            self.log(f"📍 [F2 좌표 등록 완료] 내부 배열 반영 -> waypoints[{idx}] = {self.waypoints[idx]}")
            self.waypoint_record_idx = (idx + 1) % 6
            winsound.Beep(1400, 200)
        else:
            self.log("⚠️ [좌표 기록 실패] 현재 캐릭터 위치 인식이 불가능합니다.")
            winsound.Beep(800, 300)
    ...
    def reset_waypoints(self):
        ...
        for idx in range(6):
            self.update_waypoint_ui(idx)
        self.log("🧹 순환 좌표 전체 초기화 완료")
        winsound.Beep(1000, 150)

    def save_current_profile(self):
        n = self.profile_combo.currentText().strip()
        if not n: return
        self.profiles_data[n] = {
            "reg": {"t": self.reg_t, "l": self.reg_l, "w": self.reg_w, "h": self.reg_h},
            "range": {"min": self.x_min_slider.value(), "max": self.x_max_slider.value(), "stat": self.stat_range_slider.value()},
            "keys": {
            ...
            "params": {
                "margin": 0, "precision": 0.0, "ad": self.attack_delay_ms, "dd": self.dash_delay_ms, 
                "mode": self.hunt_mode, "per_int": self.periodic_interval_min, "sell_int": self.sell_interval_min, 
                "sound": self.use_sound_alert, "shape_anti": self.use_shape_anti, "input_mode": self.input_mode,
            ...
            "coord": {
                ...
                "use_multifloor": self.use_multifloor_hunt, "multifloor_up_count": self.multifloor_up_count,
                "waypoints": self.waypoints,
                "waypoint_record_idx": self.waypoint_record_idx,
                "teleport_threshold": self.teleport_threshold
            }
        }
        self.profiles_data["_last_selected"] = n
        with open(CONFIG_FILE, "w", encoding="utf-8") as f: json.dump(self.profiles_data, f, ensure_ascii=False, indent=4)
        self.update_profile_list(); QMessageBox.information(self, "저장", "설정이 영구 저장됨")
    ...
    def apply_profile_data(self, n): 
        ...
        # 인식 정밀도 0.0 고정
        self.precision_slider.setValue(0)
        self.precision_val = 0.0
        self.color_margin = 0
        
        self.att_slider.setValue(p["ad"]); self.dash_slider.setValue(p["dd"])
        self.pet_slider.setValue(p["per_int"]); self.sell_slider.setValue(p["sell_int"])
        self.chk_alert.setChecked(p["sound"]); self.chk_shape_anti.setChecked(p["shape_anti"])
        
        # 라디오 버튼 체크 상태 변경
        mode_val = p.get("mode", 0)
        self.hunt_mode = mode_val
        if mode_val == 0:
            self.radio_lr.setChecked(True)
        elif mode_val == 1:
            self.radio_stat.setChecked(True)
        ...
        self.chk_top.setChecked(p.get("top", True))
        ...
        # 복층 사냥 설정 로드
        self.chk_multifloor.setChecked(co.get("use_multifloor", False))
        self.multifloor_up_slider.setValue(co.get("multifloor_up_count", 1))
 
        # 순환사냥 V2 설정 복원
        self.waypoints = co.get("waypoints", [(-1, -1)] * 6)
        self.waypoint_record_idx = co.get("waypoint_record_idx", 0)
        self.teleport_threshold = co.get("teleport_threshold", 20)
        self.tele_thresh_slider.setValue(self.teleport_threshold)
        for idx in range(6):
            self.update_waypoint_ui(idx)
    ...
    def load_all_profiles(self):
        try:
            ...
            self.profiles_data = {}
            self.log(f"⚠️ 설정 로드 실패 (초기화 진행): {str(e)}")

        # 기본 프로파일 생성
        if not self.profiles_data or len([k for k in self.profiles_data.keys() if not k.startswith("_")]) == 0:
            self.profiles_data["기본설정"] = {
                "reg": {"t": 100, "l": 100, "w": 200, "h": 150},
                "range": {"min": 20, "max": 180, "stat": 15},
                "keys": {"att": "end", "jump": "alt", "teleport": "shift", "pet": "del"},
                "params": {"margin": 0, "precision": 0.0, "ad": 500, "dd": 1500, "mode": 0, "per_int": 5, "sell_int": 15, "sound": True, "shape_anti": False, "input_mode": 0},
                "coord": {
        ...
        last_sel = self.profiles_data.get("_last_selected", "")
        valid_keys = [k for k in self.profiles_data.keys() if not k.startswith("_")]
        
        if last_sel in self.profiles_data and not last_sel.startswith("_"):
        ...
        self.log(f"💾 설정 자동 복원 완료 -> 프로필: {target_profile}")

    def update_profile_list(self):
        self.profile_combo.blockSignals(True)
        self.profile_combo.clear()
        
        names = [k for k in self.profiles_data.keys() if not k.startswith("_")]
        self.profile_combo.addItems(sorted(names))
        
        last_sel = self.profiles_data.get("_last_selected", "")
        if last_sel in self.profiles_data and not last_sel.startswith("_"):
            self.profile_combo.setCurrentText(last_sel)
        elif names:
    ...
    def on_profile_change(self, text):
        t = text.strip()
        if t in self.profiles_data and not t.startswith("_"):
            self.apply_profile_data(t)
            self.profiles_data["_last_selected"] = t
            try:
                with open(CONFIG_FILE, "w", encoding="utf-8") as f:
                    json.dump(self.profiles_data, f, ensure_ascii=False, indent=4)
            except:
                pass
            self.log(f"📂 프로필 변경 적용 완료 -> {t}")

    def on_hunt_mode_radio_changed(self, btn):
        idx = self.btn_group_mode.id(btn)
        self.hunt_mode = idx
        mode_names = ["좌우 이동", "제자리 사냥", "순환사냥 V2"]
        self.log(f"🔄 작동 모드 변경: {mode_names[idx] if idx < len(mode_names) else idx}")
        self.update_mode_ui_states()
    ...
    def update_window_flags(self, state):
        if state:
            ...
            self.setWindowFlags(self.windowFlags() & ~Qt.WindowStaysOnTopHint)
        self.show()
        self.log(f"📌 창 항상 맨 위 고정: {'활성화' if state else '비활성화'}")

    def update_x_min(self, v):
        self.x_min = v
        self.log(f"⚙️ 좌측 경계 임계값 변경 -> {v}")

    def update_x_max(self, v):
        self.x_max = v
        self.log(f"⚙️ 우측 경계 임계값 변경 -> {v}")

    def update_stat_range(self, v):
        self.stationary_range = v
        self.log(f"⚙️ 제자리 범위 임계값 변경 -> {v}")
