...
class CombatController:
    ...
    def execute_normal_combat(self, now_ms, last_att_time, last_dash_time):
        ...
        # 회수 중이거나 복귀 중이거나 공격 금지층(회수층 등)인 경우 공격 중단 및 키 해제
        if is_recovering or is_falling_recovering or prevent_attack:
            if getattr(self.main_win, 'is_pingpong_fixed_attacking', False):
                att_key = self.main_win.key_att_cb.currentText()
                self.main_win.set_key_state(att_key, False)
                self.main_win.is_pingpong_fixed_attacking = False
                self.main_win.logger.log("회수/복귀/공격금지 상태 진입으로 인한 공격 키 홀드 해제", "COMBAT")
            return l_att, l_dash
    ...
    def execute_pet_buff(self, last_buff_time):
        ...
        if time.time() - l_del >= self.main_win.periodic_interval_min * 60:
            self.main_win.logger.log("주기적 펫 소모품 버프 실행", "SYSTEM")
            self.main_win.press_key(self.main_win.key_pet_cb.currentText())
            l_del = time.time()
        return l_del
