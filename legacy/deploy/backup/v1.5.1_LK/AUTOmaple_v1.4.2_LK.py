...
class AUTOmapleV9_7(QMainWindow):
    ...
    def setup_ui(self):
        ...
        tab_v2_vbox.addWidget(self.reset_waypoints_btn)
        tab_v2_vbox.addStretch()
        self.mode_tabs.addTab(tab_v2_widget, "순환사냥 V2")
                     ...
                     try:
                        ...
                        if cx != -1 and cy != -1:
                            ...
                            # D. 순환사냥 V2 연산
                                ...
                                if not any(v[0] == target_idx for v in valid_pts):
                                    # 최초 시작 시 또는 목표 유실 시 가장 가까운 목적지를 우선 검색
                                    nearest_start = min(valid_pts, key=lambda v: np.hypot(cx - v[1][0], cy - v[1][1]))
                                    target_idx = nearest_start[0]
                                    self.current_waypoint_idx = target_idx
                                    self.log(f"🗺️ [V2 시작] 현재 위치에서 가장 가까운 좌표 설정 -> 슬롯 {target_idx+1}")
                                    
                                tx, ty = self.waypoints[target_idx]
                                dx = cx - tx
                                dy = cy - ty
                                
                                # 목표 좌표 안착 판정 (X 오차 ±4px, Y 오차 ±4px 이내)
                                if abs(dx) <= 4 and abs(dy) <= 4:
                                    self.set_key_state('left', False)
                                    self.set_key_state('right', False)
                                    self.set_key_state('up', False)
                                    self.log(f"🎯 순환 좌표 슬롯 {target_idx+1} 도달 완료 -> 공격 개시")
                                    ...
                                    # [V2 개선] 다음 목표를 1-2-3 순서가 아닌 가장 근접한(최단 유클리드 거리) 슬롯으로 동적 전이
                                    # 단, 방금 완료한 target_idx는 바로 다시 지목되지 않도록 후보에서 제외 (유효 좌표가 1개인 경우 제외)
                                    candidates = [v for v in valid_pts if v[0] != target_idx] if len(valid_pts) > 1 else valid_pts
                                    
                                    def calc_dist(pt):
                                        return np.hypot(cx - pt[1][0], cy - pt[1][1])
                                        
                                    best_candidate = min(candidates, key=calc_dist)
                                    self.current_waypoint_idx = best_candidate[0]
                                    
                                    self.log(f"🗺️ 최단 거리 목표 탐색 완료 -> 슬롯 {self.current_waypoint_idx+1} (X:{self.waypoints[self.current_waypoint_idx][0]}, Y:{self.waypoints[self.current_waypoint_idx][1]})")
                                    time.sleep(0.1)
                                    continue
                                ...
                                # 1. X축 정렬 우선 처리 (X 값을 먼저 맞춘 후 Y축 이동 판단)
                                if abs(dx) > 4:
                                    if abs(dx) >= 20:
                                        # 간극 20 이상 -> 텔레포트 고속 이동 (방향 전환 확실히 선행)
                                        if dx > 0:
                                ...
                                # 2. X축이 어느 정도 맞으면 (abs(dx) <= 4), Y축 상승/낙하 기동 수행
                                else:
                                    self.set_key_state('left', False)
                                    self.set_key_state('right', False)
                                    
                                    # Y축 수직 텔레포트 상승이 필요할 때 (나보다 위에 있고 5px 초과)
                                    if dy > 5:
                                        self.log(f"🪜 수직 상승 이동 기동 (Y차이: {dy}px)")
                                        self.set_key_state('up', True)
                                        time.sleep(0.08)
                                        self.press_key(teleport_key)
                                        time.sleep(0.2)
                                    ...
                                    # Y축 하단 낙하 기동이 필요할 때 (나보다 아래에 있고 5px 초과)
                                    elif dy < -5:
                                        self.log(f"⬇️ 하단 낙하 기동 (Y차이: {-dy}px)")
                                        jump_key = self.key_jump_cb.currentText()
                                        self.set_key_state('down', True)
                                        time.sleep(0.08)
                                        self.press_key(jump_key)
                            ...
                            if self.hunt_mode != 1:
                                ...
                                if self.hunt_mode == 0 and now - l_dash >= self.dash_delay_ms: self.press_key(self.key_teleport_cb.currentText()); l_dash = now
    ...
    def hotkey_record_waypoint(self):
        if self.char_x != -1 and self.char_y != -1:
            ...
            self.lbl_waypoints[idx].setText(f"슬롯 {idx+1}: X: {self.char_x}, Y: {self.char_y}")
    ...
    def reset_waypoints(self):
        ...
        self.current_waypoint_idx = 0
        for idx in range(6):
            self.lbl_waypoints[idx].setText(f"슬롯 {idx+1}: X: -1, Y: -1")
        self.log("🧹 순환 좌표 슬롯이 모두 초기화되었습니다.")
    ...
    def update_precision(self, v): self.precision_val = 0.0; self.color_margin = 0
    ...
    def update_use_multifloor(self, b): self.use_multifloor_hunt = b
    def update_multifloor_up_count(self, v): self.multifloor_up_count = v
    ...
    def save_current_profile(self): 
        ...
        self.profiles_data[n] = {
            ...
            "params": {"margin": 0, "precision": 0.0, "ad": self.attack_delay_ms, "dd": self.dash_delay_ms, "mode": self.hunt_mode, "per_int": self.periodic_interval_min, "sell_int": self.sell_interval_min, "sound": self.use_sound_alert, "shape_anti": self.use_shape_anti, "input_mode": self.input_mode},
            "coord": {
    ...
    def apply_profile_data(self, n): 
        ...
        for idx in range(6):
            ...
            if wx != -1 and wy != -1:
                self.lbl_waypoints[idx].setText(f"슬롯 {idx+1}: X: {wx}, Y: {wy}")
            else:
                self.lbl_waypoints[idx].setText(f"슬롯 {idx+1}: X: -1, Y: -1")
