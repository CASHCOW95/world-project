...
class AUTOmapleV9_7(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.chk_fall_recovery = QCheckBox("추락 감지 시 복귀 (윗점프 시퀀스)")
                        ...
                        if cx != -1 and cy != -1:
                            ...
                            if self.use_fall_recovery and cy >= self.fall_y_threshold:
                                if not self.is_falling_recovering:
                                    self.log(f"🚨 캐릭터 추락 감지 (Y: {cy} >= 임계값: {self.fall_y_threshold}). 복귀 동작 개시!")
