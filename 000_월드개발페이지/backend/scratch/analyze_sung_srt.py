...
def main():
    sung_dir = SUNG_SUBTITLE_DIR
    if not sung_dir.exists():
        print(f"Directory {sung_dir} does not exist.")
        return
        
    srt_files = sorted(sung_dir.glob('*.srt'))
    ...
    output_path = OUTPUT_DIR / 'sung_analysis_report.json'
    output_path.write_text(json.dumps(analysis_report, ensure_ascii=False, indent=2), encoding='utf-8')
        
    print(f"Seong Jeong-gil analysis report saved to {output_path}.")
