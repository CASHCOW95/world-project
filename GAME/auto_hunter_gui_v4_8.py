...
# 설정 파일 경로
CONFIG_FILE = "hunter_premium_v4_8_config.json"
...
class AutoHunterV4_8:
    def __init__(self, root):
        self.root = root
        self.root.title("오토헌터 프리미엄 v4.8 (주기설정 추가)")
        ...
        self.use_periodic = tk.BooleanVar(value=True) # DEL 키 사용 여부
        self.periodic_interval_min = tk.IntVar(value=5) # 주기(분)
    ...
    def setup_scrollable_ui(self):
        ...
        # 3. 주기 및 간격
        group3 = ttk.LabelFrame(container, text=" [ 3. 상세 주기 설정 ] ", padding="10")
        ...
        # DEL 키 주기
        ttk.Label(group3, text="DEL 키 입력 주기 (분):").pack(anchor=tk.W, pady=(5, 0))
        p_f = ttk.Frame(group3); p_f.pack(fill=tk.X)
        ttk.Scale(p_f, from_=1, to=60, variable=self.periodic_interval_min, orient=tk.HORIZONTAL).pack(side=tk.LEFT, expand=True, fill=tk.X)
        ttk.Entry(p_f, textvariable=self.periodic_interval_min, width=6, font=("Malgun Gothic", 10, "bold")).pack(side=tk.LEFT, padx=5)
                    ...
                    try:
                        ...
                        if not found and self.is_anti_macro_working:
                            if self.anti_lost_time < 20:
                                ...
                                self.is_anti_macro_working = False; self.last_anti_pos = None; self.log("[정보] 거탐 종료")
                    except: pass
                time.sleep(0.05)
    ...
    def toggle_running(self):
        if self.is_running:
            self.is_running = False; self.start_btn.config(text="사냥 시작 (F12)", bg="#00adb5"); self.log("사냥 중단 (OFF)")
            pyautogui.keyUp('left'); pyautogui.keyUp('right')
        else:
            self.is_running = True; self.start_btn.config(text="사냥 중지 (F12)", bg="#ff4b2b"); self.log("사냥 시작 (ON)!")
                    ...
                    if np.any(mask):
                        ...
                        if M["m00"] > 0:
                            ...
                            if self.hunt_mode.get() == 1:
                                if start_cx is None: start_cx = cx; self.log(f"기준점: {cx}")
                                t_min, t_max = start_cx - 5, start_cx + 5
                                if cx >= t_max and cur_dir == 'right': pyautogui.keyUp('right'); cur_dir = 'left'
                                elif cx <= t_min and cur_dir == 'left': pyautogui.keyUp('left'); cur_dir = 'right'
                            ...
                            # 신규: 주기적 DEL 키 (체크박스 + 간격 반영)
                            if self.use_periodic.get():
                                interval_sec = self.periodic_interval_min.get() * 60
                                if (time.time() - l_del_time) >= interval_sec:
                                    self.log(f"[주기] DEL 키 2연타 ({self.periodic_interval_min.get()}분)")
...
if __name__ == "__main__":
    root = tk.Tk(); app = AutoHunterV4_8(root); root.mainloop()
