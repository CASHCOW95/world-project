...
# 설정 파일 경로
CONFIG_FILE = "hunter_premium_v5_0_config.json"
...
class AutoHunterV5_0:
    def __init__(self, root):
        self.root = root
        self.root.title("오토헌터 프리미엄 v5.0 (안정화 최종판)")
    ...
    def setup_scrollable_ui(self):
        ...
        # 2. 기능 제어
        group2 = ttk.LabelFrame(container, text=" [ 2. 핵심 기능 제어 ] ", padding="10")
                    ...
                    if np.any(mask):
                        ...
                        if M["m00"] > 0:
                            ...
                            if self.hunt_mode.get() == 1: # 제자리 사냥
                                if start_cx is None: start_cx = cx; self.log(f"제자리 시작점: {cx}")
                            ...
                            if self.use_periodic.get() and (time.time() - l_del_time) >= (self.periodic_interval_min.get() * 60):
                                self.log(f"[주기] DEL 키 입력 ({self.periodic_interval_min.get()}분)"); self.press_key(self.KEY_PERIODIC); time.sleep(0.2); self.press_key(self.KEY_PERIODIC); l_del_time = time.time()
...
if __name__ == "__main__":
    root = tk.Tk(); app = AutoHunterV5_0(root); root.mainloop()
